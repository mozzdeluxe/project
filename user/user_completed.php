...
// ดึงงานที่ส่งแล้ว
$query = "
    SELECT a.assign_id, a.status, j.job_title, j.job_description, j.due_datetime, 
           m.firstname, m.lastname 
    FROM assignments a 
    JOIN jobs j ON a.job_id = j.job_id
    JOIN mable m ON j.supervisor_id = m.id  -- ใช้ j.supervisor_id แทน a.supervisor_id
    WHERE a.user_id = ? 
    AND a.status IN ('ยังไม่อ่าน','อ่านแล้ว','กำลังดำเนินการ','ส่งแล้ว','ช้า') 
                ...
                <tbody id="jobTable">
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        while ($row = mysqli_fetch_assoc($result)) {
                            $status_class = ($row['status'] == 'ช้า' || strpos($row['status'], 'ช้า') !== false) ? 'text-danger' : 'text-success';
                            echo '<tr>';
